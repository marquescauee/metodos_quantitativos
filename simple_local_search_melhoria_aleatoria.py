        ...
        for _ in range(max_iterations):
            ...
            while candidates:
                ...
                while(iterations_inside_neighbors_limit <= current_vertice_length):
                    #vizinho selecionado aleatoriamente
                    neighbor = random.choice(graph.get(current_vertice))

                    #ordem do vizinho
                    neighbor_length = len(graph.get(neighbor))

                    #se a ordem do vizinho é menor que a do vértice atual e se o vizinho for um candidato, substitui o current_vertice a ser iterado
                    if(neighbor_length < current_vertice_length and neighbor in candidates):
                        current_vertice = neighbor
                        current_vertice_length = len(graph.get(neighbor))
                        iterations_inside_neighbors_limit = 0
